...
import CardMenuPopup from "@/app/pages/projects/mtg-db/CardMenuPopup";
...
export default function CardHome() {
    ...
    const [currentCard, setCurrentCard] = useState<Scry.Card>(null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setCardResults(await Scry.Cards.search(queryString, 1).cancelAfterPage().waitForAll());
    ...
    function buildCardGrid(cards: Scry.Card[]) {
        return (
            <div className = "mt-16 flex flex-row flex-wrap justify-center gap-4">
                {cards.map((card) => (
                    <>
                    <Image key={card.id} onClick={() => (handleCardClick(card.id)) } className={"rounded-3xl w-full  max-h-[420px] max-w-[300px] hover:cursor-pointer"} width={300} height={200}  src = {card.image_uris?.normal || card.card_faces[0].image_uris?.normal}  alt = {card.name}/>
    ...
    return (
        <>
            <div >
                <span className = "wtext-center ml-8 text-2xl text-yellow-500 font-bold"> This is a port from jQuery to React. Still a work in Progress! <Link className="underline" href={"https://www.seanslaughter.dev/class-env/pages/project/Card-Database/card-home.php"}>View On Older Site!</Link></span>
